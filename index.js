...
function App() {
  const [currentPage, setCurrentPage] = useState('home'); // 'home', 'login', 'register', 'new-post'
  ...
  const PostList = ({ db, user }) => {
    ...
    const handleDelete = async (postId, postAuthorId) => {
      ...
      if (!window.confirm('Are you sure you want to delete this post?')) { // Using window.confirm for simplicity, replace with custom modal in production
        return;
      }
      try {
        ...
        <nav className="space-x-4">
          ...
          {user ? (
            <>
              ...
              <button onClick={async () => { await signOut(auth); setUser(null); setUserId(null); setCurrentPage('home'); }} className="bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-lg transition duration-200">Logout</button>
            </>
          ) : (
