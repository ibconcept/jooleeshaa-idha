...
import { getFirestore, collection, addDoc, onSnapshot, query, orderBy, deleteDoc, doc, updateDoc, arrayUnion, arrayRemove, serverTimestamp } from 'firebase/firestore';
...
function App() {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        await addDoc(collection(db, `artifacts/${appId}/public/data/posts`), {
          ...
          confirmedBy: []   // Initialize confirmedBy array
        });
    ...
    return (
      <div className="max-w-xl mx-auto p-6 bg-gray-800 rounded-xl shadow-lg mt-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="locationText" className="block text-gray-300 text-sm font-bold mb-2">Location (e.g., "Main Street", "Near Park"):</label>
            <input
              type="text"
              id="locationText"
              value={locationText}
              ...
              className="shadow appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline bg-gray-700 border-gray-600 text-white"
              placeholder="Enter a descriptive location"
            />
            <button
              type="button"
              onClick={handleGetLocation}
              className="mt-2 bg-gray-600 hover:bg-gray-700 text-white text-sm py-1 px-3 rounded-lg transition duration-200"
            >
              Get Current GPS Location
            </button>
            {latitude && longitude && (
              <p className="text-gray-400 text-xs mt-2">GPS: {latitude.toFixed(4)}, {longitude.toFixed(4)}</p>
            )}
          </div>
          <button
            type="submit"
  ...
  const PostList = ({ db, user }) => {
    ...
    const handleConfirm = async (postId, currentConfirmedBy) => {
      if (!user) {
        setMessage('You must be logged in to confirm a post.');
        setMessageType('error');
        return;
      }
      if (currentConfirmedBy && currentConfirmedBy.includes(user.uid)) {
        setMessage('You have already confirmed this post.');
        setMessageType('error');
        return;
      }
      ...
      try {
        ...
        await updateDoc(postRef, {
          confirmations: (currentConfirmedBy ? currentConfirmedBy.length : 0) + 1,
        ...
        setMessage('Failed to confirm post: ' + error.message);
        setMessageType('error');
      }
    };
      ...
      <div className="max-w-2xl mx-auto mt-8 space-y-6">
        ...
        {posts.map((post) => (
          <div key={post.id} className="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
            ...
            {(post.locationText || (post.latitude && post.longitude)) && (
              <p className="text-gray-400 text-sm mb-2">
                Location: {post.locationText}
                {post.latitude && post.longitude && ` (GPS: ${post.latitude.toFixed(4)}, ${post.longitude.toFixed(4)})`}
              </p>
            )}
            ...
            <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-700">
              <div className="flex items-center space-x-2">
                <span className="text-gray-400 text-sm">Confirms: {post.confirmations || 0}</span>
                {user && (!post.confirmedBy || !post.confirmedBy.includes(user.uid)) && (
                  <button
                    onClick={() => handleConfirm(post.id, post.confirmedBy)}
                    className="bg-blue-600 hover:bg-blue-700 text-white text-xs py-1 px-3 rounded-lg transition duration-200"
                  >
                    Confirm
                  </button>
                )}
                {user && post.confirmedBy && post.confirmedBy.includes(user.uid) && (
                  <span className="text-green-400 text-xs">You confirmed this!</span>
                )}
              </div>
